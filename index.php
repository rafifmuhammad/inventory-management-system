...
$today = date('Y-m-d');
// Query informasi dashboard
$dashboard_info = getData("SELECT 
    (SELECT COUNT(*) FROM tb_barang) AS jumlah_barang,
    (SELECT IFNULL(SUM(jumlah), 0) FROM tb_barang_masuk) AS jumlah_barang_masuk,
    (SELECT IFNULL(SUM(jumlah), 0) FROM tb_barang_keluar) AS jumlah_barang_keluar,
    (SELECT IFNULL(SUM(jumlah), 0) FROM tb_detail_penjualan WHERE created_at = '$today') AS barang_terjual;
")[0];
...
SELECT 
    ...
    (
        SELECT MIN(tanggal_kadaluwarsa)
        FROM tb_barang_masuk
        WHERE id_barang = b.id_barang
    ) AS expired_terdekat
...
foreach ($products as $product) {
    ...
    $expired = $product['expired_terdekat'];

    if ($stok <= 0) {
        $status[] = "<button class='btn btn-gradient-danger btn-sm'>Habis</button>";
    }

    elseif ($stok <= $min) {
        $status[] = "<button class='btn btn-gradient-warning btn-sm'>Menipis</button>";
    }

    if (!empty($expired)) {

        if ($expired < $today) {
            $status[] = "<button class='btn btn-gradient-danger btn-sm'>Kadaluarsa</button>";
        }

        elseif ($expired <= $six_month) {
            $status[] = "<button class='btn btn-gradient-warning btn-sm'>Mendekati Kadaluarsa</button>";
        }
    }

    if (!empty($status)) {
        $product['status_badge'] = implode(' ', $status);
        ...
        <div class="container-fluid">
            ...
            <div class="col-lg-3">
              <div class="card card-eco">
                <div class="card-body">
                  <div class="row">
                    <div class="col-8">
                      <h4 class="title-text mt-0">Barang Dimiliki</h4>
                      <h3 class="font-weight-semibold mb-1"><?= $dashboard_info['jumlah_barang']; ?></h3>
                    </div>
                    <!--end col-->
                    <div class="col-4 text-center align-self-center">
                      <!-- <span class="card-eco-icon">👳🏻</span> -->
                      <i
                        class="dripicons-archive card-eco-icon align-self-center"
                      ></i>
                    </div>
                    <!--end col-->
                  </div>
            ...
            <div class="col-lg-3">
              <div class="card card-eco">
                <div class="card-body">
                  <div class="row">
                    <div class="col-8">
                      <h4 class="title-text mt-0">Barang Masuk</h4>
                      <h3 class="font-weight-semibold mb-1"><?= $dashboard_info['jumlah_barang_masuk']; ?></h3>
                    </div>
                    <!--end col-->
                    <div class="col-4 text-center align-self-center">
                      <!-- <span class="card-eco-icon">🛒</span> -->
                      <i
                        class="dripicons-store card-eco-icon align-self-center"
                      ></i>
                    </div>
                    <!--end col-->
                  </div>
            ...
            <div class="col-lg-3">
              <div class="card card-eco">
                <div class="card-body">
                  <div class="row">
                    <div class="col-8">
                      <h4 class="title-text mt-0">Barang Keluar</h4>
                      <h3 class="font-weight-semibold mb-1"><?= $dashboard_info['jumlah_barang_keluar']; ?></h3>
                    </div>
                    <!--end col-->
                    <div class="col-4 text-center align-self-center">
                      <!-- <span class="card-eco-icon">🎲</span> -->
            ...
            <div class="col-lg-3">
              <div class="card card-eco">
                <div class="card-body">
                  <div class="row">
                    <div class="col-8">
                      <h4 class="title-text mt-0">Terjual Hari ini</h4>
                      <h3 class="font-weight-semibold mb-1"><?= $dashboard_info['barang_terjual']; ?></h3>
                    </div>
                    <!--end col-->
                    <div class="col-4 text-center align-self-center">
                      <!-- <span class="card-eco-icon">💰</span> -->
          ...
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-body">
                  <h4 class="mt-0 header-title">Tabel Peringatan</h4>
                  ...
                  >
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td><?= $product['expired_terdekat'] ?? '-'; ?></td>
                        <td><?= number_format($product['stok'], 0, ',', '.'); ?></td>
                        <td><?= number_format($product['stok_minimum'], 0, ',', '.'); ?></td>
